...
const ImageFormRendererWrapper = memo(
  ({
    isWizardOpen,
    stableKey,
    initialValues,
    ...
    return (
      <FormRenderer
        ...
        onCancel={onClose}
      >
        <BlueprintListenerWrapper />
      </FormRenderer>
    );
  }
);

ImageFormRendererWrapper.displayName = "ImageFormRendererWrapper";

ImageFormRendererWrapper.propTypes = {
...
const CreateImageWizard = (props) => {
  ...
  const handleOpen = useCallback(() => {
    ...
    setStableKey(`image-wizard-${Date.now()}`);
  }, []);

  const handleBuildImage = useCallback(
    async (fields, formAPI) => {
        ...
        if (
          ...
          uploadSettings = {
            ...
            settings: {
              ...
              storageAccessKey:
                formValues.image.upload.settings.storageAccessKey,
  ...
  const initialValues = useMemo(() => {
    return props.blueprint ? blueprintToFormState(props.blueprint) : {};
  }, [props.blueprint?.name, isWizardOpen]);
